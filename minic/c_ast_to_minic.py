...
def maybe_special_unary(orig):
    return {
        ...
        '++': (lambda x: mc.Assignment(x, mc.BinaryOp('+', x, mc.Constant('int', '1')), nid=get_new_id()))
    }.get(orig.op, lambda x: mc.UnaryOp(orig.op, x, nid=get_new_id()))(transform(orig.expr))
...
def transform(x):
    return {
        ...
        c_ast.ArrayRef: (lambda orig: mc.ArrayRef(transform(orig.name), transform(orig.subscript), nid=get_new_id())),
        ...
        c_ast.DoWhile: (lambda orig: mc.DoWhile(transform(orig.cond), transform(orig.stmt), coord=orig.coord,
                                                nid=get_new_id())),
        c_ast.EmptyStatement: (lambda orig: mc.EmptyStatement(nid=get_new_id())),
        c_ast.ExprList: (lambda orig: mc.ExprList(tmap(orig.exprs), nid=get_new_id())),
        c_ast.FileAST: (lambda orig: mc.FileAST(lmap(transform, orig.ext), nid=get_new_id())),
        ...
        c_ast.FuncCall: (lambda orig: mc.FuncCall(transform(orig.name), tmap(orig.args), nid=get_new_id())),
        c_ast.FuncDecl: (lambda orig: mc.FuncDecl(tmap(orig.args), transform(orig.type), nid=get_new_id())),
        c_ast.FuncDef: (lambda orig: mc.FuncDef(transform(orig.decl), tmap(orig.param_decls), transform(orig.body),
                                                nid=get_new_id())),
        c_ast.ID: (lambda orig: mc.ID(v(orig.name), nid=get_new_id())),
        ...
        c_ast.If: (lambda orig: mc.If(transform(orig.cond), transform(orig.iftrue), transform(orig.iffalse),
                                      coord=orig.coord, nid=get_new_id())),
        c_ast.InitList: (lambda orig: mc.InitList(tmap(orig.exprs), nid=get_new_id())),
        c_ast.NamedInitializer: (lambda orig: mc.NamedInitializer(v(orig.name), transform(orig.expr),
                                                                  nid=get_new_id())),
        c_ast.ParamList: (lambda orig: mc.ParamList(tmap(orig.params), nid=get_new_id())),
        c_ast.PtrDecl: (lambda orig: mc.PtrDecl(transform(orig.type), nid=get_new_id())),
        c_ast.Return: (lambda orig: mc.Return(transform(orig.expr), nid=get_new_id())),
        c_ast.TernaryOp: (lambda orig: mc.TernaryOp(transform(orig.cond), transform(orig.iftrue),
                                                    transform(orig.iffalse), nid=get_new_id())),
        c_ast.Typename: (lambda orig: mc.Typename(v(orig.name), transform(orig.type), nid=get_new_id())),
        c_ast.TypeDecl: (lambda orig: mc.TypeDecl(v(orig.declname), transform(orig.type), nid=get_new_id())),
